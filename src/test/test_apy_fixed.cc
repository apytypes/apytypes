...
TEST_CASE("APyFixed assumptions") {
  // The GMP `mp_limb_t` data type is either 64 bit or 32 bit. Any other limb
  // size is unsupported. This assumption should hold true always, according to
  // the GMP documentation, but check it just to be safe.
...
TEST_CASE(
    "APyFixed::bits(), APyFixed::int_bits() and APyFixed::vector_size()") {
...
TEST_CASE("APyFixed::twos_complement_overflow()") {
  REQUIRE(APyFixed(128, 1, to_limb_vec({0x0, 0x8000000000000000}))
              .to_string_dec() == "-1");
  REQUIRE(APyFixed(128, 1, to_limb_vec({0x0, 0x4000000000000000}))
              .to_string_dec() == "0.5");
  REQUIRE(
...
TEST_CASE("APyFixed::_data_asl()") {
  { /* Test #1 */
    ...
    REQUIRE(fix._data_asl(33) ==
            to_limb_vec({0xFFFFFFFE00000000, uint64_t(-1)}));
  }

  { /* Test #2 */
    APyFixed fix(128, 128,
                 to_limb_vec({0xDEADBEEFDEADBEEF, 0x7FFFFFFBADBADBAD}));
    REQUIRE(fix._data_asl(0) == fix._data);
    REQUIRE(fix._data_asl(256 + 4 * 10) ==
            to_limb_vec({0, 0, 0, 0, 0xADBEEF0000000000, 0xBADBADDEADBEEFDE,
                         0x7FFFFFFBAD}));
  }
...
TEST_CASE("APyFixed::from_double()") {
  ...
  REQUIRE(APyFixed(-123.0, 64, 64).to_string_dec() == "-123");
  REQUIRE(APyFixed(std::pow(2.0, 52.0) + 0, 64, 64).to_string_dec() ==
          "4503599627370496");
  REQUIRE(APyFixed(std::pow(2.0, 52.0) + 1, 64, 64).to_string_dec() ==
          "4503599627370497");
  REQUIRE(APyFixed(std::pow(2.0, 53.0) + 0, 64, 64).to_string_dec() ==
          "9007199254740992");
  REQUIRE(APyFixed(std::pow(2.0, 53.0) + 1, 64, 64).to_string_dec() ==
          "9007199254740992" // Precision was lost when adding 1 onto 2^53
  );
  REQUIRE(APyFixed(std::pow(2.0, 123.0) - std::pow(2.0, 152), 30, 153)
              .to_string_dec() ==
          "-5708990760190015557953816894567524063288229888");
  REQUIRE(APyFixed(std::pow(2.0, 123.0) - std::pow(2.0, 152), 29, 153)
              .to_string_dec() ==
  ...
  REQUIRE(APyFixed(-std::pow(2, -127), 1, -126).to_string_dec() ==
          ...
          "36358642578125");
  REQUIRE(APyFixed(-1.0 + std::pow(2, -53), 55, 2).to_string_dec() ==
...
TEST_CASE(
    "Vector initialization must be consistent with APyFixed word-length") {
  // Vector-initialization fails on size missmatch
  REQUIRE_THROWS_AS(APyFixed(_LIMB_SIZE_BITS + 1, 0, to_limb_vec({0})),
                    std::domain_error);
