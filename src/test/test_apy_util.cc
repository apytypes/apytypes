...
TEST_CASE("to_nibble_list()")
{
    ...
    REQUIRE(to_nibble_list(uint64_vec{ 0 }) == uint8_vec{ 0 });

    // Simple tests
    REQUIRE(to_nibble_list(uint64_vec{ 1 }) == uint8_vec{ 1 });
    REQUIRE(to_nibble_list(uint64_vec{ 15 }) == uint8_vec{ 15 });
    REQUIRE(to_nibble_list(uint64_vec{ 16 }) == uint8_vec{ 0, 1 });
    REQUIRE(to_nibble_list(uint64_vec{ 0xff }) == uint8_vec{ 0x0f, 0x0f });
    REQUIRE(to_nibble_list(uint64_vec{ 0xfca }) == uint8_vec{ 0x0a, 0x0c, 0x0f });

    // Longer test
    REQUIRE(
        to_nibble_list(uint64_vec{
            0xabcd0012,
            0x773300,
            0x7E00AD0000BE00EF,
        }) == uint8_vec{
...
{
    { /* Zero-element */
        auto nibble_list = to_nibble_list(uint64_vec{ 0x00 });
        bool out_bit = nibble_list_shift_left_once(nibble_list);
        REQUIRE(nibble_list == uint8_vec{ 0 });
        REQUIRE(out_bit == 0);
    }
    { /* Simple shift */
        auto nibble_list = to_nibble_list(uint64_vec{ 0x88F7 });
        bool out_bit = nibble_list_shift_left_once(nibble_list);
        REQUIRE(nibble_list == uint8_vec{ 0x0E, 0x0E, 0x01, 0x01 });
        REQUIRE(out_bit == 1);
    }
    { /* More complex shift */
        auto nibble_list = to_nibble_list(uint64_vec{
            0xF000070F0056789A,
            0x13373,
            18,
            0xFFFFF00
...
TEST_CASE("double_dabble()")
{
    ...
    REQUIRE(double_dabble(uint64_vec{ 0 }) == uint8_vec{ 0 });
    REQUIRE(double_dabble(uint64_vec{ 1 }) == uint8_vec{ 1 });
    REQUIRE(double_dabble(uint64_vec{ 9 }) == uint8_vec{ 9 });
    REQUIRE(double_dabble(uint64_vec{ 10 }) == uint8_vec{ 0, 1 });

    // Problematique test #1
    REQUIRE(double_dabble(uint64_vec{ 0x0ff }) == uint8_vec{ 5, 5, 2 });
    REQUIRE(double_dabble(uint64_vec{ 0x100 }) == uint8_vec{ 6, 5, 2 });
    REQUIRE(double_dabble(uint64_vec{ 0x0ffff }) == uint8_vec{ 5, 3, 5, 5, 6 });
    REQUIRE(double_dabble(uint64_vec{ 0x10000 }) == uint8_vec{ 6, 3, 5, 5, 6 });

    // Larger tests
    REQUIRE(
        double_dabble(uint64_vec{ 0x7ff003011fff2fef })
        == uint8_vec{
            3, 8, 5, 3, 7, 0, 4, 9, 5, 0, 4, 7, 1, 7, 8, 8, 1, 2, 9
        }
    );
    REQUIRE(
        double_dabble(uint64_vec{ 0x00FFFFFFFFFA70CE, 0x1234500 })
        == uint8_vec{
