...
static inline std::vector<mp_limb_t> to_limb_vec(std::vector<uint64_t> vec) {
  static_assert(sizeof(mp_limb_t) == 4    // 32-bit target architecture
...
significant_limbs(const std::vector<mp_limb_t> &vector) {
  ...
  auto rev_non_zero_it =
      std::find_if(vector.crbegin(), vector.crend(), is_non_zero);
  return std::distance(vector.begin(), rev_non_zero_it.base());
}
...
to_nibble_list(const std::vector<mp_limb_t> &data_array, std::size_t len = 0) {
  ...
  auto is_non_zero = [](auto n) { return n != 0; };
  auto non_zero_it =
      std::find_if(result.rbegin(), result.rend() - len, is_non_zero);
...
static inline bool
nibble_list_shift_left_once(std::vector<uint8_t> &nibble_list) {
...
static inline bool
nibble_list_shift_right_once(std::vector<uint8_t> &nibble_list) {
  if (nibble_list.size() == 0) {
    return false;
  }
...
struct DoubleDabbleList {
  ...
  static constexpr mp_limb_t _NIBBLE_MASK =
      _LIMB_SIZE_BITS == 64 ? 0x1111111111111111 // 64-bit architecture
...
double_dabble(std::vector<mp_limb_t> nibble_data) {
  ...
  auto is_non_zero = [](auto n) { return n != 0; };
  nibble_data.erase(
      std::find_if(nibble_data.rbegin(), nibble_data.rend() - 1, is_non_zero)
          .base(),
      nibble_data.end());
  ...
  const mp_limb_t new_bit_mask =
      nibbles_last_limb == 0 ? mp_limb_t(1) << (_LIMB_SIZE_BITS - 1)
...
reverse_double_dabble(const std::vector<uint8_t> &bcd_list) {
  ...
  while (
      ...
      std::any_of(bcd.data.begin(), bcd.data.end(),
                  [](auto n) { return n != 0; }) ||
      iteration % 4 != 0) {
...
static inline void limb_vector_asr(std::vector<mp_limb_t> &vec,
                                   unsigned shift_amnt) {
...
static inline void limb_vector_lsr(std::vector<mp_limb_t> &vec,
                                   unsigned shift_amnt) {
  if (!vec.size() || !shift_amnt) {
    return;
  }
...
static inline void limb_vector_lsl(std::vector<mp_limb_t> &vec,
                                   unsigned shift_amnt) {
  if (!vec.size() || !shift_amnt) {
    return;
  }
