...
public:
    ...
    ) const
    {
        ...
        if (dim == tuple.size() - 1) {
            ...
            if (std::holds_alternative<nb::int_>(element)) {
                ...
                auto src = input_it + _itemsize * idx * strides[dim];
                std::copy_n(src, _itemsize * strides[dim], output_it);
                return strides[dim];
                ...
                if (step < 0) {
                    for (std::ptrdiff_t src_i = start; src_i > stop; src_i += step) {
                        auto src = input_it + _itemsize * src_i * strides[dim];
                        auto dst = output_it + _itemsize * elements_copied;
                        std::copy_n(src, _itemsize * strides[dim], dst);
    ...
    get_item_ndarray(const nb::ndarray<bool, nb::c_contig>& key) const
    {
        ...
        for (std::size_t i = 0; i < key.size(); i++) {
            if (key.data()[i]) {
                auto src = std::begin(_data) + i * chunk_size * _itemsize;
                auto dst = std::begin(res._data) + i_dst++ * chunk_size * _itemsize;
                std::copy_n(src, chunk_size * _itemsize, dst);
            }
        }
    ...
    )
    {
        ...
        if (dim == key.size() - 1) {
            ...
            if (std::holds_alternative<nb::int_>(element)) {
                ...
                auto dst = output_it + _itemsize * strides[dim] * idx;
                std::copy_n(input_it, _itemsize * strides[dim], dst);
                return strides[dim];
            } else { /* std::holds_alternative<nb::slice>(element) */
                auto&& slice = std::get<nb::slice>(element);
                auto [start, stop, step, len] = slice.compute(_shape[dim]);
                ...
                if (step < 0) {
                    for (std::ptrdiff_t dst_i = start; dst_i > stop; dst_i += step) {
                        auto src = input_it + _itemsize * elements_copied;
                        auto dst = output_it + _itemsize * dst_i * strides[dim];
                        std::copy_n(src, _itemsize * strides[dim], dst);
                        elements_copied += strides[dim];
                    }
                } else { /* step >= 0 */
        ...
        for (std::size_t i = 0; i < key.size(); i++) {
            if (key.data()[i]) {
                for (std::size_t j = 0; j < chunk_size; j++) {
                    std::size_t offset = i * chunk_size * _itemsize + j * _itemsize;
                    val.copy_n_to(std::begin(_data) + offset, _itemsize);
                }
            }
        }
    }
    ...
    )
    {
        ...
        if (slice_ndim == 1 && val._ndim > 1) {
            // Special case for Numpy slicing compatibility. Only when the slice ndim is
            // one and `val` ndim is greater than one, do we throw a `TypeError` here.
            // If the slice ndim is greater than one, we might first need to squeeze of
            // dimensions of `val` before determening success or failure.
            std::string err_msg = fmt::format(
                "{}.__setitem__: ndarray boolean slicing requires 1-dimensional input, "
                "but input has {} dimensions",
                ARRAY_TYPE::ARRAY_NAME,
                val._ndim
            );
        ...
        // Count of number of `true` values of the key array
        std::size_t n_true = std::count(key.data(), key.data() + key.size(), true);

        // Compute the slice shape
        std::vector<std::size_t> slice_shape(slice_ndim);
        slice_shape[0] = std::max(n_true, std::size_t(1));
        std::size_t ndim_cpy = slice_ndim - 1;
        std::copy_n(std::end(_shape) - ndim_cpy, ndim_cpy, std::begin(slice_shape) + 1);

        // The chunk size, i.e., the number of elements to copy for each `true` value in
        // the slice array
        std::size_t n_chunk = fold_shape(std::end(_shape) - ndim_cpy, std::end(_shape));

        // Start and sentinel iterator pair of `*this` shape squeezed off leading ones
        auto squeeze_end = std::end(val._shape);
        auto squeeze_begin = std::find_if(
            std::begin(val._shape), std::end(val._shape) - 1, [](auto dim) {
                return dim != 1;
            }
        );

        bool is_squeezed_shape_equal = std::equal(
            std::begin(slice_shape), std::end(slice_shape), squeeze_begin, squeeze_end
        );
        if (val._shape == slice_shape || is_squeezed_shape_equal) {
            // The slice exactly matches the shape of `val`. Simply copy the data
            std::size_t slice_i = 0;
            for (std::size_t i = 0; i < key.size(); i++) {
                if (key.data()[i]) {
                    auto src = std::begin(val._data) + slice_i++ * n_chunk * _itemsize;
                    auto dst = std::begin(_data) + i * n_chunk * _itemsize;
                    std::copy_n(src, n_chunk * _itemsize, dst);
                }
            }
            return;
        }

        // Create vector of the squeezed `val` shape
        std::vector<std::size_t> squeeze_shape(squeeze_begin, squeeze_end);

        if (is_broadcastable(squeeze_shape, slice_shape)) {
            // The elements of `val` can be broadcast to match the slice
            vector_type broadcast(_itemsize * n_chunk * slice_shape[0]);
            broadcast_data_copy(
                std::begin(val._data), // src
                std::begin(broadcast), // dst
                squeeze_shape,         // src_shape
                slice_shape,           // dst_shape
                _itemsize              // itemsize
            );
            std::size_t slice_i = 0;
            for (std::size_t i = 0; i < key.size(); i++) {
                if (key.data()[i]) {
                    auto src = std::begin(broadcast) + slice_i++ * n_chunk * _itemsize;
                    auto dst = std::begin(_data) + i * n_chunk * _itemsize;
                    std::copy_n(src, n_chunk * _itemsize, dst);
                }
            }
        ...
        // The shape of `val` is not compatible with the slice shape
        std::string err_msg = fmt::format(
            ...
            tuple_string_from_vec(slice_shape)
        );
        throw nb::value_error(err_msg.c_str());
    }
