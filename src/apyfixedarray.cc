...
APyCFixedArray APyFixedArray::rsubc(const APyCFixed& lhs) const
{
...
APyFixedArray APyFixedArray::rdiv(const APyFixed& lhs) const
{
    ...
    if (unsigned(res_bits) <= 2 * APY_LIMB_SIZE_BITS) {
        ...
        if (unsigned(lhs.bits()) <= APY_LIMB_SIZE_BITS) {
            numerator = (__int128)(apy_limb_signed_t)lhs._data[0];
        } else {
            numerator = (__int128)lhs._data[0];
            numerator |= (__int128)(apy_limb_signed_t)lhs._data[1]
                << APY_LIMB_SIZE_BITS;
        }
        numerator <<= bits();
        if (unsigned(bits()) <= APY_LIMB_SIZE_BITS) {
    ...
    if (unsigned(res_bits) <= 2 * APY_LIMB_SIZE_BITS) {
        ...
        if (unsigned(lhs.bits()) <= APY_LIMB_SIZE_BITS) {
            numerator = (std::int64_t)(apy_limb_signed_t)lhs._data[0];
        } else {
            numerator = (std::int64_t)lhs._data[0];
            numerator |= (std::int64_t)(apy_limb_signed_t)lhs._data[1]
                << APY_LIMB_SIZE_BITS;
        }
        numerator <<= bits();
        if (unsigned(bits()) <= APY_LIMB_SIZE_BITS) {
...
APyCFixedArray APyFixedArray::rdivc(const APyCFixed& lhs) const
{
...
)
{
    ...
    if (ndim == 0) {
        throw nb::value_error(
            "APyFixedArray.from_array: zero-dimensional arrays not supported"
        );
    }
        ...
        if (ndarray.dtype() == nb::dtype<__TYPE__>()) {                                \
            ...
            for (std::size_t i = 0; i < ndarray.size(); i++) {                         \
                fixed_point_from_integer(                                              \
                    static_cast<__TYPE__>(ndarray_view.data()[i]),                     \
                    std::begin(_data) + (i + 0) * _itemsize,                           \
                    std::begin(_data) + (i + 1) * _itemsize,                           \
                    _bits,                                                             \
                    _int_bits                                                          \
