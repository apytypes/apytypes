...
//! Iterator-based less-than function, comparing `abs(src1) < abs(src2)` when `src1` and
//! `src2` have the same bit specifiers
[[maybe_unused]] static APY_INLINE bool floating_point_less_than_abs_same_wl(
    const APyFloatData& src1, const APyFloatData& src2, const APyFloatSpec& spec
)
{
    if (is_nan(src1, spec.exp_bits) || is_nan(src2, spec.exp_bits)) {
        return false;
    }
...
)
{
    ...
    bool swap = floating_point_less_than_abs_same_wl(*src1, *src2, spec);
    ...
    if (x.exp == MAX_EXP || y.exp == MAX_EXP) {
        ...
        if (x_is_nan || y_is_nan || (both_inf && x_sign != y_sign)) {
            // Set to NaN
            z = { x_sign, exp_t(MAX_EXP), man_t(1) };
            return;
        } else {
            // Set to inf
            bool sign = x.man == 0 ? x_sign : y_sign;
            z = { sign, exp_t(MAX_EXP), man_t(0) };
            return;
        }
    }
...
struct FloatInnerProdFunctor {
    ...
    const decltype(get_qntz_func(_qntz)) _qntz_func = get_qntz_func(_qntz);
    ...
    const int MAN_DELTA = SUM_MAN_BITS + 2 - _dst_spec.man_bits;
    const man_t STICKY = (1ULL << (MAN_DELTA - 1)) - 1;
