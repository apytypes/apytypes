...
public:
    ...
    APY_INLINE bool same_type_as(const APyCFloatArray& other) const noexcept
    {
        return spec() == other.spec();
    }

    //! Test if `*this` has the same bit specifiers as another `APyCFloat`
    APY_INLINE bool same_type_as(const APyCFloat& other) const noexcept
    {
        return spec() == other.spec();
    }
...
public:
    ...
    //! Create an `APyCFloatArray` with evenly spaced values within a given interval
    static APyCFloatArray arange(
