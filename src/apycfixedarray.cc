...
APyCFixedArray APyCFixedArray::rsub(const APyCFixed& lhs) const
{
        ...
        if (frac_bits() == lhs.frac_bits()) {
            ...
            if (rhs_shift_amount) {
                ...
                for (std::size_t i = 0; i < _nitems * 2; i += 2) {
                    result._data[i + 0]
                        = lhs._data[0] - (_data[i + 0] << rhs_shift_amount);
                    result._data[i + 1]
                        = lhs._data[1] - (_data[i + 1] << rhs_shift_amount);
                }
            } else {
                auto lhs_real = lhs._data[0] << lhs_shift_amount;
                auto lhs_imag = lhs._data[1] << lhs_shift_amount;
                VECTORIZE_LOOP
                for (std::size_t i = 0; i < _nitems * 2; i += 2) {
                    result._data[i + 0] = lhs_real - _data[i + 0];
                    result._data[i + 1] = lhs_imag - _data[i + 1];
                }
            }
        }
        return result; // early exit
    ...
    for (; result_it != result._data.end(); result_it += result._itemsize) {
        ...
        apy_inplace_reversed_subtraction_same_length( // real part
            result_it,                                // dst_begin
            result_it + result._itemsize / 2,         // dst_end
            imm.real_begin()                          // src_begin
        );
        apy_inplace_reversed_subtraction_same_length( // imaginary part
            result_it + result._itemsize / 2,         // dst_begin
            result_it + result._itemsize,             // dst_end
            imm.imag_begin()                          // src_begin
        );
    }
...
APyCFixedArray APyCFixedArray::operator*(const APyFixedArray& rhs) const
{
        ...
        if (unsigned(bits()) <= APY_LIMB_SIZE_BITS
            ...
            for (std::size_t i = 0; i < result._nitems * 2; i += 2) {
                complex_real_multiplication_1_1_2(
                    &result._data[2 * i], &_data[i], &rhs._data[i / 2]
                );
            }
            return result; // early exit
        } else if (unsigned(bits()) <= APY_LIMB_SIZE_BITS) {
            // Left-hand side fits in one limb, right-hand side uses two limbs.
            VECTORIZE_LOOP
            for (std::size_t i = 0; i < result._nitems; i++) {
                complex_real_multiplication_1_2_2(
                    &result._data[i * result._itemsize],
                    &_data[i * _itemsize],
                    &rhs._data[i * rhs._itemsize]
                );
            }
            return result; // early exit
        } else {
            // Left-hand side uses two limbs, right-hand side fits in one limb.
            VECTORIZE_LOOP
            for (std::size_t i = 0; i < result._nitems; i++) {
    ...
    // * prod_imm:      2 + 2 * src1_limbs + 2 * src2_limbs
    std::size_t src1_limbs = _itemsize / 2;
    std::size_t src2_limbs = rhs._itemsize;
    std::size_t scratch_size = 2 + 3 * src1_limbs + 3 * src2_limbs;
    ScratchVector<apy_limb_t, 64> scratch(scratch_size);
    auto op1_abs_begin = std::begin(scratch);
    auto op2_abs_begin = op1_abs_begin + src1_limbs;
    ...
    if (unsigned(res_bits) <= 2 * APY_LIMB_SIZE_BITS) {
        if (unsigned(bits()) <= APY_LIMB_SIZE_BITS) {
            if (unsigned(rhs.bits()) <= APY_LIMB_SIZE_BITS) {
                ...
                for (std::size_t i = 0; i < result._nitems * 2; i += 2) {
                    complex_multiplication_1_1_2(
                        result._data.data() + 2 * i, _data.data() + i, rhs._data.data()
                    );
                }
            } else {
                // Right-hand side does not fit in single limb, but left-hand side does.
            ...
            for (std::size_t i = 0; i < result._nitems * 2; i += 2) {
                complex_multiplication_1_2_2(
                    result._data.data() + 2 * i, rhs._data.data(), _data.data() + 2 * i
                );
            }
        }
        return result; // early exit
...
APyCFixedArray APyCFixedArray::operator/(const APyFixed& rhs) const
{
    ...
    if (unsigned(res_bits) <= 2 * APY_LIMB_SIZE_BITS) {
        ...
        if (unsigned(rhs.bits()) <= COMPILER_LIMB_SIZE) {
            ...
            den = ((__int128)apy_limb_signed_t(rhs._data[1]) << COMPILER_LIMB_SIZE)
                | (__int128)rhs._data[0];
        }
        int rhs_bits = rhs.bits();
        if (unsigned(bits()) <= COMPILER_LIMB_SIZE) {
            VECTORIZE_LOOP
            for (std::size_t i = 0; i < 2 * result._nitems; i += 2) {
                __int128 real
                    = ((__int128)apy_limb_signed_t(_data[i + 0]) << rhs_bits) / den;
                result._data[2 * i + 0] = apy_limb_t(real);
                result._data[2 * i + 1] = apy_limb_t(real >> COMPILER_LIMB_SIZE);
                __int128 imag
                    = ((__int128)apy_limb_signed_t(_data[i + 1]) << rhs_bits) / den;
                result._data[2 * i + 2] = apy_limb_t(imag);
                result._data[2 * i + 3] = apy_limb_t(imag >> COMPILER_LIMB_SIZE);
            }
        } else {
            VECTORIZE_LOOP
            for (std::size_t i = 0; i < 4 * result._nitems; i += 4) {
                __int128 real = (__int128)apy_limb_signed_t(_data[i + 1])
                        << COMPILER_LIMB_SIZE
                    | (__int128)_data[i + 0];
                real = (real << rhs_bits) / den;
                result._data[i + 0] = apy_limb_t(real);
                result._data[i + 1] = apy_limb_t(real >> COMPILER_LIMB_SIZE);
                __int128 imag = (__int128)apy_limb_signed_t(_data[i + 3])
                        << COMPILER_LIMB_SIZE
                    | (__int128)_data[i + 2];
                imag = (imag << rhs_bits) / den;
                result._data[i + 2] = apy_limb_t(imag);
                result._data[i + 3] = apy_limb_t(imag >> COMPILER_LIMB_SIZE);
            }
        }
        return result; // early exit
#endif
        ...
        std::int64_t den;
        if (unsigned(rhs.bits()) <= COMPILER_LIMB_SIZE) {
            den = (std::int64_t)apy_limb_signed_t(rhs._data[0]);
        } else {
            den = ((std::int64_t)apy_limb_signed_t(rhs._data[1]) << COMPILER_LIMB_SIZE)
                | (std::int64_t)rhs._data[0];
        }
        int rhs_bits = rhs.bits();
        if (unsigned(bits()) <= COMPILER_LIMB_SIZE) {
            VECTORIZE_LOOP
            for (std::size_t i = 0; i < 2 * result._nitems; i += 2) {
                std::int64_t real
                    = ((std::int64_t)apy_limb_signed_t(_data[i + 0]) << rhs_bits) / den;
                result._data[2 * i + 0] = apy_limb_t(real);
                result._data[2 * i + 1] = apy_limb_t(real >> COMPILER_LIMB_SIZE);
                std::int64_t imag
                    = ((std::int64_t)apy_limb_signed_t(_data[i + 1]) << rhs_bits) / den;
                result._data[2 * i + 2] = apy_limb_t(imag);
                result._data[2 * i + 3] = apy_limb_t(imag >> COMPILER_LIMB_SIZE);
            }
        } else {
            VECTORIZE_LOOP
            for (std::size_t i = 0; i < 4 * result._nitems; i += 4) {
                std::int64_t real = (std::int64_t)apy_limb_signed_t(_data[i + 1])
                        << COMPILER_LIMB_SIZE
                    | (std::int64_t)_data[i + 0];
                real = (real << rhs_bits) / den;
                result._data[i + 0] = apy_limb_t(real);
                result._data[i + 1] = apy_limb_t(real >> COMPILER_LIMB_SIZE);
                std::int64_t imag = (std::int64_t)apy_limb_signed_t(_data[i + 3])
                        << COMPILER_LIMB_SIZE
                    | (std::int64_t)_data[i + 2];
                imag = (imag << rhs_bits) / den;
                result._data[i + 2] = apy_limb_t(imag);
                result._data[i + 3] = apy_limb_t(imag >> COMPILER_LIMB_SIZE);
            }
    ...
    for (std::size_t i = 0; i < _nitems; i++) {
        auto res_real = std::begin(result._data) + i * result._itemsize;
        auto res_imag = res_real + result._itemsize / 2;
        auto src_real_begin = std::begin(_data) + i * _itemsize;
        auto src_real_end = src_real_begin + _itemsize / 2;
        auto src_imag_begin = src_real_end;
        auto src_imag_end = src_imag_begin + _itemsize / 2;
...
APyCFixedArray APyCFixedArray::operator/(const APyCFixedArray& rhs) const
{
    ...
    if (unsigned(div_bits) <= 2 * APY_LIMB_SIZE_BITS) {
        ...
        //          rhs.frac_bits()) + rhs.bits() = 1 + (int_bits() + frac_bits()) +
        //          2 * rhs.bits()
...
APyCFixedArray APyCFixedArray::rdiv_real(const APyFixed& lhs) const
{
    ...
    if (unsigned(div_bits) <= APY_LIMB_SIZE_BITS) {
        ...
        for (std::size_t i = 0; i < result._nitems * 2; i += 2) {
            apy_limb_signed_t den_real = apy_limb_signed_t(_data[i + 0]);
    ...
    if (unsigned(div_bits) <= 2 * APY_LIMB_SIZE_BITS) {
        ...
        if (unsigned(lhs.bits()) <= APY_LIMB_SIZE_BITS) {
            ...
            __int128 num_imag = (__int128)(lhs._data[2])
                | (__int128)apy_limb_signed_t(lhs._data[3]) << APY_LIMB_SIZE_BITS;
    ...
    if (unsigned(div_bits) <= 2 * APY_LIMB_SIZE_BITS) {
        ...
        if (unsigned(lhs.bits()) <= APY_LIMB_SIZE_BITS) {
            ...
            std::int64_t num_imag = (std::int64_t)(lhs._data[2])
                | (std::int64_t)apy_limb_signed_t(lhs._data[3]) << APY_LIMB_SIZE_BITS;
...
) const
{
    ...
    APyCFixedArray::vector_type result_data(
        checked_size_mul(checked_size_mul(_nitems, 2), result_limbs) + pad_limbs
    );
    ...
    result_data.resize(checked_size_mul(checked_size_mul(_nitems, 2), result_limbs));
...
APyCFixedArray APyCFixedArray::conj() const
{
    ...
    if (res._itemsize == 2) {
        // Single limb specialization
        simd::vector_conj(res._data.begin(), _data.begin(), 2 * res._nitems);
...
APyCFixedArray::prod(const std::optional<PyShapeParam_t>& py_axis) const
{
    ...
    if (n_elems == 0) {
        ...
        // Non-empty array
        int int_bits = (_int_bits - 1) * n_elems + 2 + (n_elems + 1) / 2;
...
APyCFixedArray APyCFixedArray::cumprod(std::optional<nb::int_> py_axis) const
{
    ...
    // Compute the result word length
    int int_bits
        = std::max(int((_int_bits - 1) * n_elems + 2 + (n_elems + 1) / 2), _int_bits);
...
void APyCFixedArray::_set_bits_from_ndarray(const nb::ndarray<nb::c_contig>& ndarray)
{
    ...
    // `_check_and_set_bits_from_ndarray` returns true. Otherwise, the next
    // attempted conversion will take place
...
#define CHECK_AND_SET_VALUES_FROM_INT_NPTYPE(__TYPE__)                                 \
    ...
    // converts all the data if it matches. If successful, the macro returns.
    // Otherwise, the next attempted conversion will take place.
...
) const
{
    ...
    std::copy(res_arr._data.begin(), res_arr._data.end(), res._data.begin());
    return res;
}
...
) const
{
    ...
    // Let `a` be a pointer to the longer array, and let `b` be a pointer to the
    // reverse copy of the shorter array.
