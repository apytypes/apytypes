...
static void gmp_default_free(void* p, size_t unused_size) { free(p); }
...
static mp_ptr gmp_alloc_limbs(mp_size_t size)
{
    return (mp_ptr)gmp_alloc(size * sizeof(mp_limb_t));
}

static mp_ptr gmp_realloc_limbs(mp_ptr old, mp_size_t old_size, mp_size_t size)
{
    assert(size > 0);
    return (mp_ptr
    )gmp_realloc(old, old_size * sizeof(mp_limb_t), size * sizeof(mp_limb_t));
}

static void gmp_free_limbs(mp_ptr old, mp_size_t size)
{
...
int mpn_zero_p(mp_srcptr rp, mp_size_t n) { return mpn_normalized_size(rp, n) == 0; }
...
void mpn_sqr(mp_ptr rp, mp_srcptr ap, mp_size_t n) { mpn_mul(rp, ap, n, ap, n); }
...
static mp_bitcnt_t
mpn_common_scan(mp_limb_t limb, mp_size_t i, mp_srcptr up, mp_size_t un, mp_limb_t ux)
{
    unsigned cnt;
...
mp_bitcnt_t mpn_scan1(mp_srcptr ptr, mp_bitcnt_t bit)
{
    ...
    return mpn_common_scan(
        ptr[i] & (GMP_LIMB_MAX << (bit % GMP_LIMB_BITS)), i, ptr, i, 0
    );
}
...
mp_bitcnt_t mpn_scan0(mp_srcptr ptr, mp_bitcnt_t bit)
{
    ...
    return mpn_common_scan(
        ~ptr[i] & (GMP_LIMB_MAX << (bit % GMP_LIMB_BITS)), i, ptr, i, GMP_LIMB_MAX
    );
}
...
static mp_limb_t mpn_div_qr_1_preinv(
    mp_ptr qp, mp_srcptr np, mp_size_t nn, const struct gmp_div_inverse* inv
)
{
    mp_limb_t d, di;
    mp_limb_t r;
...
static void mpn_div_qr_2_preinv(
    mp_ptr qp, mp_ptr np, mp_size_t nn, const struct gmp_div_inverse* inv
)
{
    unsigned shift;
    mp_size_t i;
...
mpn_get_str_bits(unsigned char* sp, unsigned bits, mp_srcptr up, mp_size_t un)
{
    ...
    sn = ((un - 1) * GMP_LIMB_BITS + mpn_limb_size_in_base_2(up[un - 1]) + bits - 1)
        / bits;
...
/* We generate digits from the least significant end, and reverse at
   the end. */
static size_t
mpn_limb_get_str(unsigned char* sp, mp_limb_t w, const struct gmp_div_inverse* binv)
{
    mp_size_t i;
    for (i = 0; w > 0; i++) {
        mp_limb_t h, l, r;
...
static size_t mpn_get_str_other(
    unsigned char* sp,
    int base,
...
static mp_size_t
mpn_set_str_bits(mp_ptr rp, const unsigned char* sp, size_t sn, unsigned bits)
{
    mp_size_t rn;
    mp_limb_t limb;
    unsigned shift;
...
static mp_size_t mpn_set_str_other(
    mp_ptr rp,
...
size_t mpz_size(const mpz_t u) { return GMP_ABS(u->_mp_size); }
...
mp_srcptr mpz_limbs_read(mpz_srcptr x) { return x->_mp_d; }
...
mp_ptr mpz_limbs_write(mpz_t x, mp_size_t n) { return mpz_limbs_modify(x, n); }
...
int mpz_sgn(const mpz_t u) { return GMP_CMP(u->_mp_size, 0); }
...
static void mpz_div_q_2exp(
    mpz_t q, const mpz_t u, mp_bitcnt_t bit_index, enum mpz_div_round_mode mode
)
{
    mp_size_t un, qn;
    mp_size_t limb_cnt;
    ...
    if (mode == ((un > 0) ? GMP_DIV_CEIL : GMP_DIV_FLOOR)) /* un != 0 here. */
        ...
        adjust
            = (qn <= 0 || !mpn_zero_p(u->_mp_d, limb_cnt)
               || (u->_mp_d[limb_cnt] & (((mp_limb_t)1 << bit_index) - 1)));
    else
        adjust = 0;
...
static void mpz_div_r_2exp(
    mpz_t r, const mpz_t u, mp_bitcnt_t bit_index, enum mpz_div_round_mode mode
)
{
    mp_size_t us, un, rn;
    mp_ptr rp;
...
static unsigned long mpz_div_qr_ui(
    mpz_t q, mpz_t r, const mpz_t n, unsigned long d, enum mpz_div_round_mode mode
)
{
    unsigned long ret;
    mpz_t rr, dd;
...
void mpz_sqrtrem(mpz_t s, mpz_t r, const mpz_t u) { mpz_rootrem(s, r, u, 2); }

void mpz_sqrt(mpz_t s, const mpz_t u) { mpz_rootrem(s, NULL, u, 2); }
...
void mpz_2fac_ui(mpz_t x, unsigned long n) { mpz_mfac_uiui(x, n, 2); }

void mpz_fac_ui(mpz_t x, unsigned long n) { mpz_mfac_uiui(x, n, 1); }
...
static int
gmp_millerrabin(const mpz_t n, const mpz_t nm1, mpz_t y, const mpz_t q, mp_bitcnt_t k)
{
    assert(k > 0);
