...
def test_repr():
    assert (
        ...
        == r"APyCFixed((44, 0), int_bits=2, frac_bits=4)"
    )
    assert (
        repr(APyCFixed.from_float(-1.25 + 1j, int_bits=2, frac_bits=4))
        == r"APyCFixed((44, 16), int_bits=2, frac_bits=4)"
    )
    assert (
        repr(APyCFixed.from_float(-1.25 - 1j, int_bits=2, frac_bits=4))
        == r"APyCFixed((44, 48), int_bits=2, frac_bits=4)"
    )
    assert (
        APyCFixed(-1, int_bits=12, frac_bits=4).__repr__()
        == r"APyCFixed((65535, 0), int_bits=12, frac_bits=4)"
    )
    assert (
        APyCFixed(1234, int_bits=12, frac_bits=0).__repr__()
        == r"APyCFixed((1234, 0), int_bits=12, frac_bits=0)"
    )
    assert (
        APyCFixed.from_float(-2.666, int_bits=3, frac_bits=52).__repr__()
        == r"APyCFixed((24022200412394226, 0), int_bits=3, frac_bits=52)"
    )
    ...
    assert (
        ...
        == "APyCFixed((164, 0), int_bits=4, frac_bits=4)"
    )
