...
async function run_apytypes_test_harness(apytypes_wheel_name: string) {
    ...
    const lib_zip_url: string = "http://localhost:8000/lib.zip";
    console.log(`[ HARNESS ]: fetching "lib/test" via "${lib_zip_url}"`);
    try {
        const resp = await fetch(lib_zip_url);
        const buf = await resp.arrayBuffer();
        pyodide.unpackArchive(buf, "zip");
        console.log(`[ HARNESS ]: successfully retrieved and unpacked "lib/test"`);
    } catch (e) {
        console.log(`[ HARNESS ]: failed to provide "lib/test"`);
